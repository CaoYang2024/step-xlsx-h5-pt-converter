...
def do_matplotlib(X, Y, Z_vis, lumi=None, out_png="height3d.png"):
    ...
    fc = None
    if lumi is not None:
        l = (lumi - np.nanmin(lumi)) / max(1e-12, (np.nanmax(lumi) - np.nanmin(lumi)))
        import matplotlib.cm as cm
        fc = cm.gray(l)
        if hasattr(Zm, "mask") and isinstance(Zm.mask, np.ndarray):
            fc = ma.array(fc, mask=np.repeat(Zm.mask[:, :, None], 4, axis=2))

    ax.plot_surface(X, Y, Zm, rstride=1, cstride=1, linewidth=0, antialiased=True,
                    facecolors=fc, cmap=None if fc is not None else "viridis")
    ax.set_xlabel("X (mm)")
    ax.set_ylabel("Y (mm)")
...
# -------- new: side-by-side height & lumi figure --------
def save_side_by_side_height_lumi(X, Y, Z_vis, L, out_png="height_lumi_cmp.png",
    ...
    if Lm is not None:
        ...
        if hasattr(Zm, "mask") and isinstance(Zm.mask, np.ndarray):
            fc = ma.array(fc, mask=np.repeat(Zm.mask[:, :, None], 4, axis=2))
        ax2.plot_surface(X, Y, Zm, rstride=1, cstride=1, linewidth=0,
                         antialiased=True, facecolors=fc, cmap=None)
        mappable = cm.ScalarMappable(cmap="gray")
